...
```
<!--
Updated: 2021-01-27 by Chaeyeon Han @ University of Illinois at Urbana-Champaign UP431 Spring 2021
...
```{r, message = FALSE, warning=FALSE}
trips <- read_sav("Data/spss (2)/trippub.sav")
```
...
#total_commute <- aggregate(mode_share_commute$n, by = list(mode_share_commute$commuteTrp),sum) 

#mode_share_commute <- mode_share_commute %>% mutate(pct = ifelse(commuteTrp=="commute_trip",n/total_commute[1,2],n/total_commute[2,2]))
...
ggplot(mode_share_commute, aes(x = mode_short, y = per)) +
 # geom_bar(aes(fill = commuteTrp), position = "dodge", stat = 'identity') +
...
chi_trips <- chi_trips %>% 
  ...
  mutate(mode_short_carpool = ifelse(mode_short == "Auto" & NUMONTRP > 2,"Carpool", as.character(mode_short)))
